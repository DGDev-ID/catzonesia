...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: `Detail #${props.transaction.id}`, href: `/cashier/transaction/${props.transaction.id}` },
];
...
const makeSuccess = () => {
    router.post(route('cashier.markAsSuccess', props.transaction.id));
};

const makeFailed = () => {
    router.post(route('cashier.markAsFailed', props.transaction.id));
};
</script>

<template>
    <Head title="Transaction Detail (Kasir)" />
            ...
            <div class="max-w-7xl mx-auto px-6 space-y-8">
                <div class="flex items-center justify-between gap-4">
                    ...
                    <div class="flex items-center gap-2">
                        <button
                            type="button"
                            @click="makeSuccess"
                            class="inline-flex items-center gap-2 justify-center rounded-xl bg-green-600 px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:opacity-90"
                        >
                            <Check :size="16" />
                            Make Success
                        </button>
                        <button
                            type="button"
                            @click="makeFailed"
                            class="inline-flex items-center gap-2 justify-center rounded-xl bg-red-600 px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:opacity-90"
                        >
                            <X :size="16" />
                            Make Failed
                        </button>
                        <Link href="/cashier" class="text-sm text-muted-foreground hover:text-foreground transition">
                            ← Kembali
                        </Link>
                    </div>
                </div>
                ...
                <div class="rounded-2xl border bg-background shadow-sm p-8">
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            <p class="mb-1 text-xs font-medium text-muted-foreground uppercase tracking-wide">Kode Unik</p>
                            <p class="text-sm font-medium">{{ props.transaction.unique_id }}</p>
                        </div>
                        <div>
